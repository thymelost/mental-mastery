...
**Research base:** 7 papers reviewed (see `research-grounding.md`). Revisions from research marked with `[R]` inline.
...
| B2.4 | Identity Stability Monitoring | Track drift from baseline self over time — across hours, days, extended influence. Slow drift is the hardest to detect; this skill exists specifically for it. | B2.3, S1.3 | P3.2 |
...
4. **B2.4 (Identity Stability Monitoring) is the most exposed skill.** Slow drift is the hardest attack surface. It requires longitudinal data — progress tracking is not optional for this skill.
